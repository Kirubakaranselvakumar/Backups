<!-- <?php
        ...
        if ($patient_id) {
            $sql = "UPDATE patients SET first_name=?, middle_name=?, last_name=?, dob=?, mobile=?, email=?, gender=?, city=?, district=?, state=?, country=?, pincode=?, blood_group=?,weight=?, height=?, bmi=?, marital_status=? WHERE patient_id=?";
            ...
            $sql = "INSERT INTO patients (first_name, middle_name, last_name, dob, mobile, email, gender, city, district, state, country, pincode, blood_group, height, weight, bmi, marital_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
